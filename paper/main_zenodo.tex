...
\newcommand{\AvA}{\RIA}  % legacy alias for any leftover macro use
\newcommand{\MHA}{\mathrm{MHA}}

\title{Role-Indexed Attention:\\
Typed Edges for Compositional Binding in Transformers}

\author{%
  Rishi Vhare\\
  Independent researcher\\
  \texttt{lucifermorsbio@gmail.com}
}
...
introduce \textbf{Role-Indexed Attention (RIA)}, a factored attention
mechanism whose every edge is typed by a triple $(r, p, q)$ drawn
from learned vocabularies of $R$ relations and $K$ aspect slots. This
mirrors the standard structure of frame semantics
\citep{fillmore1976,baker1998framenet}, description-logic roles with
typed domain and range \citep{baader2003description}, and the
predicate--argument structure used in semantic role labelling
\citep{palmer2005propbank}. We use that structure as an architectural
prior and evaluate the mechanism as a neural method.
%
RIA recovers standard multi-head attention as its $K{=}1$ reduction
(verified to $10^{-7}$ in code) and tensor-product role-filler binding
as its $R{=}1$ reduction. On a controlled conjunction-binding
diagnostic, RIA reaches \textbf{100.0 / 100.0\%} (IID / weak-OOD)
...
\emph{never appear in training contexts at all}, RIA still attains
\textbf{99.1\%} OOD accuracy while MHA collapses to
\textbf{35.8\%}. A quantitative interpretability index
shows that RIA's $(r, p, q)$ edges spontaneously specialize: the top
...
($d{=}256, L{=}6$) RIA reaches \textbf{86.7\%} weak-OOD vs MHA's
\textbf{33.5\%}; at \textbf{44M parameters} ($d{=}512, L{=}8$) with a
big-model recipe (lr $= 10^{-3}$, 300k examples), RIA reaches
\textbf{100.0\% IID and 98.4\% strict-OOD} while parameter-matched MHA
reaches only \textbf{42\%}. On a 2-hop chained-binding task,
the architectural advantage compounds with depth: from $L{=}6$
(38.5\%) to $L{=}12$, RIA's OOD accuracy reaches \textbf{69.6\% mean}
and \textbf{81.5\% best-seed} --- a +31\,pp gain, while MHA and a
parameter-matched MHA-wide gain less than 8\,pp. On the
\textbf{SCAN} compositional generalisation benchmark
\citep{lake2018scan}, RIA reaches \textbf{99.7\%} sequence-exact
accuracy on the standard split (vs.\ MHA's 95.2\% and MHA-wide's
97.9\%, mean over 3 seeds); on the canonical hard \texttt{addprim\_jump}
split, all three methods fail at $<$1\% sequence-exact, consistent
with the Lake \& Baroni (2018) finding that vanilla transformer
variants fail on novel-primitive composition. We are explicit about
this: typed-edge attention solves \emph{conjunctive binding}, not
all forms of compositional generalisation. Reference implementation
runs on a laptop CPU; the full GPU battery completes on a single T4.
\end{abstract}
...
two distinctions that cognitive science, linguistics, and classical predicate logic treat as separate:
\begin{enumerate}
...
The combination is the natural generalization of multi-head
attention along the structural axis that frame semantics
\citep{fillmore1976}, description logic \citep{baader2003description},
and semantic role labelling \citep{palmer2005propbank} preserve. We use these structures as a source of inductive bias: the
architecture is designed to match the typed-relation factorization,
but the contribution is evaluated as a neural method.

\paragraph{Contributions.}
\begin{enumerate}
    \item \textbf{Role-Indexed Attention (RIA)} --- a drop-in attention
      layer whose attention tensor has the shape
      $[B, R, N, K, N, K]$ instead of $[B, H, N, N]$ (\S\ref{sec:method}).
    \item A \textbf{subsumption and separation theorem} showing RIA
    ...
    \item Empirical results on a conjunction-binding diagnostic:
      RIA reaches 100\% on both IID and weak-OOD; under \emph{strict
      OOD} (held-out pairs absent from \emph{any} training example),
      RIA still generalises while parameter-matched MHA collapses to
      the weak-attention plateau (\S\ref{sec:experiments}).
    \item A \textbf{role-specialisation index} showing RIA's
...
\section{Background: typed relations in three traditions}
\label{sec:background}

The architectural commitments in \S\ref{sec:method} are independent of
this section, which only motivates the design. Three established
formalisms factor relational structure in the way we mirror in the
attention mechanism.

\paragraph{Frame semantics.}
Fillmore's frame semantics \citep{fillmore1976,fillmore1982} and its
operationalisation in FrameNet \citep{baker1998framenet} represent
each predicate as a \emph{frame} together with a fixed set of
\emph{frame elements}. A frame element is a typed slot: the
\textsc{Commerce\_buy} frame, for example, has elements
\textsc{Buyer}, \textsc{Seller}, \textsc{Goods}, \textsc{Money},
each constrained to a specific kind of filler. A sentence
instantiates a frame by binding fillers to elements. The same noun
can fill different elements in different frames without contradiction.

\paragraph{Description logic.}
Description-logic knowledge bases \citep{baader2003description}
distinguish concepts (unary types) from \emph{roles} (binary
relations). Each role $R$ has a domain and a range, which restrict
the types of entities it can connect: e.g.\ \textsc{authorOf} has
domain \textsf{Person} and range \textsf{Document}. The standard
OWL knowledge representation framework is built on this
domain/range typing of roles.

\paragraph{Predicate--argument structure.}
Semantic role labelling \citep{palmer2005propbank,
gildea2002automatic} decomposes a sentence into a predicate plus
arguments, each argument tagged with a semantic role
(\textsc{Arg0}, \textsc{Arg1}, etc.\ in PropBank; thematic roles
in earlier traditions). The same word fills different roles in
different sentences; the structure of the predicate--argument
relation is the load-bearing object.

In all three formalisms, every relational connection between two
entities is specified not by a single scalar but by a triple:
\begin{itemize}
    \item the \textbf{relation type} $r$ (frame name; role; predicate),
    \item the \textbf{source role} $p$ in which the first entity participates,
    \item the \textbf{target role} $q$ in which the second entity participates.
\end{itemize}
Standard attention compresses all three into one scalar. This paper
asks what happens when the structure is preserved.

\section{Role-Indexed Attention}
\label{sec:method}
...
input may be read. This is faithful to frame-semantic's reading of
role-types (P2: the role-type names the aspect, it does not truncate the
cognition).

For every token pair $(i, j)$ and every role-type triple $(r, p, q) \in
[R] \times [K] \times [K]$:
\begin{align}
...
\textbf{Algorithm 1: Role-Indexed Attention (forward)}\\
\small
...
\label{thm:mha}
For any $H, d_h$ with $H d_h = d$, there exists a choice of RIA
...
\label{thm:rf}
At $R{=}1$, RIA realises a discrete-index tensor-product binding in the
...
$x$. RIA places each feature on a distinct $(r, p)$ or $(r, q)$ channel
...
frame-semantic's ``joint specification'' property (P3) would instead
...
for one forward pass at three representative shapes. RIA's overhead
is $\sim$3.5--6$\times$ MHA, scaling with the larger attention
tensor. With top-$k$ role-type gating (Appendix~\ref{app:topk}) at
...
\toprule
Shape ($d, L, N, B$) & MHA params & RIA params &
  MHA FLOPs & RIA FLOPs & wall ratio \\
\midrule
...
the constant compute overhead: for compositional binding, RIA at
...
$4 d$-wide MLP. MHA baseline uses $H=4$. RIA uses $R=4$, $K \in \{1,
...
RIA ($R{=}4, K{=}1$)  & 605,568  & 59.2 $\pm$ 20.9  & 51.9 $\pm$ 12.2 \\
RIA ($R{=}4, K{=}2$)  & 1,097,088  & 100.0 $\pm$ 0.0  & 99.9 $\pm$ 0.2 \\
RIA ($R{=}4, K{=}4$) \textbf{(ours)}  & 1,047,936  & 100.0 $\pm$ 0.0  & 100.0 $\pm$ 0.0 \\
\bottomrule
\end{tabular}
\end{table}
...
$d{=}176$ to match RIA-$(K{=}4)$'s parameter count.}
\label{tab:matched}
\begin{tabular}{lrcc}
\toprule
Method & \# params & IID acc. & OOD acc. \\
...
RIA ($R{=}4, K{=}4, d{=}128$)  & 1,047,936  & 100.0 $\pm$ 0.0  & 100.0 $\pm$ 0.0 \\
\bottomrule
\end{tabular}
\end{table}
...
RIA ($R{=}4, K{=}4$) \textbf{(ours)}  & 1,047,936  & 100.0 $\pm$ 0.0  & 99.1 $\pm$ 0.7 \\
\bottomrule
\end{tabular}
\end{table}

The strict-OOD result is the central evidence for compositional
binding. RIA generalizes to pairs it never saw because its
...
clearly: RIA reaches 86.7\% weak-OOD and 85.9\% strict-OOD, vs
MHA's $\sim$33\% (Table~\ref{tab:scaling-A}). Four of five RIA
...
\textbf{Recipe-B fixes the big-model failure for RIA, not for MHA}
(Table~\ref{tab:scaling-B}). Under the longer / lower-LR recipe,
RIA reaches \textbf{100.0\% IID and 98.4\% strict-OOD at 44M
...
$L$           & 6   & 8   & 10  & 12 \\
\midrule
RIA OOD       & 38.5\% & 48.2\% & 66.1\% & \textbf{69.6\%} (best 81.5) \\
...
non-typed alternatives' does not. From $L{=}6$ to $L{=}12$, RIA's
...
training stops helping); RIA monotonically improves throughout.
The best RIA seed at $L{=}12$ reaches \textbf{81.5\% OOD},
...
\hspace{1em} \textbf{RIA} ($K{=}4$)     & 8,295,936 & \textbf{88.9 $\pm$ 18.7} & \textbf{86.7 $\pm$ 19.3} \\
\midrule
...
\hspace{1em} \textbf{RIA} ($K{=}4$)     & 8,295,936 & \textbf{87.3 $\pm$ 23.1} & \textbf{85.9 $\pm$ 22.7} \\
\midrule
...
\hspace{1em} \textbf{RIA} ($K{=}4$)     & 44,126,208 & 33.4 $\pm$ 1.1 & 33.4 $\pm$ 1.2 \\
\bottomrule
\end{tabular}
\end{table}
...
recipe lets the big model learn for RIA but not for MHA.}
\label{tab:scaling-B}
\begin{tabular}{lrcc}
\toprule
Method & \# params & IID acc. & OOD acc. \\
...
\hspace{1em} \textbf{RIA} ($K{=}4$)     & 44,126,208 & \textbf{100.0 $\pm$ 0.0} & \textbf{98.4 $\pm$ 0.9} \\
\midrule
...
\hspace{1em} \textbf{RIA} ($K{=}4$)     & 11,053,056 & \textbf{48.3 $\pm$ 1.2} & \textbf{48.2 $\pm$ 0.9} \\
\bottomrule
\end{tabular}
\end{table}
...
Going from $L{=}8$ (recipe-B) to $L{=}10$, RIA's accuracy on
...
\hspace{1em} \textbf{RIA} ($K{=}4$)     & 13,810,176 & \textbf{65.0 $\pm$ 7.7} & \textbf{66.1 $\pm$ 10.1} \\
\bottomrule
\end{tabular}
\end{table}
...
examples, $\mathrm{lr}{=}1\!\times\!10^{-3}$, 3 seeds. RIA continues
...
\hspace{1em} \textbf{RIA} ($K{=}4$)     & 16,567,296 & \textbf{70.6 $\pm$ 8.4} & \textbf{69.6 $\pm$ 10.4} \\
\bottomrule
\end{tabular}
\end{table}
...
\]
across the four RIA depth points ($L \in \{6, 8, 10, 12\}$ at
...
\midrule
RIA  ($d{=}256$ sweep)    & $\mathbf{0.00}$ & 0.80 & 0.89 \\
...
Within the four-point regime, RIA's fit is consistent with
...
fits. RIA's fit asymptotes towards 100\% accuracy; MHA's fits
asymptote to $\sim 50\%$.}
...
We evaluate on SCAN \citep{lake2018scan}, the canonical
sequence-to-sequence compositional-generalization benchmark. Inputs
are short English-like instructions (e.g., \texttt{jump twice and
walk}); outputs are action sequences (e.g., \texttt{JUMP JUMP WALK}).
We use a 4-layer decoder-only transformer at $d{=}128$ trained as a
causal LM on \texttt{[input] <SEP> [output] <EOS>}, with loss only
on output tokens; evaluation is greedy decoding plus exact-match
sequence accuracy.
...
\begin{table}[h]
\centering
\caption{\textbf{SCAN results.} Sequence-exact accuracy on the test
set, mean $\pm$ std over 3 seeds. The \texttt{simple} split is the
random IID partition; \texttt{addprim\_jump} holds out compositions
of the primitive ``jump'' from training.}
\label{tab:scan}
\begin{tabular}{lrcc}
\toprule
Method & \# params & \texttt{simple} acc. & \texttt{addprim\_jump} acc. \\
\midrule
MHA ($d{=}128, H{=}4$)            &   801,664 & 95.2 $\pm$ 1.6 & 0.0 $\pm$ 0.1 \\
MHA wide ($d{=}176, H{=}4$)       & 1,507,792 & 97.9 $\pm$ 0.5 & 0.1 $\pm$ 0.1 \\
\textbf{RIA} ($R{=}K{=}4, d{=}128$) & 1,391,488 & \textbf{99.7 $\pm$ 0.0} & 0.2 $\pm$ 0.2 \\
\bottomrule
\end{tabular}
\end{table}

On the IID \texttt{simple} split RIA reaches \textbf{99.7\%}
sequence-exact accuracy, $4.5$\,pp above MHA at the same
$d_{\text{model}}$ and $1.8$\,pp above the parameter-matched
MHA-wide. On the harder \texttt{addprim\_jump} split, all three
methods collapse to $<1\%$, consistent with the original
\citet{lake2018scan} finding that vanilla transformer variants do
not solve novel-primitive composition. The error mode in
\texttt{addprim\_jump} is structurally different from the
conjunction-binding errors RIA was designed to address: there is no
training-time signal connecting the held-out primitive ``jump'' to
the abstract operators (\texttt{twice}, \texttt{thrice}, \texttt{and},
\dots). RIA's typed edges \emph{cannot} substitute for missing
data, only for missing inductive bias on data the model does see;
the negative result here makes that scope explicit.
...
measure, for each RIA edge type or MHA head, the
...
Three layers, $K{=}R{=}4$ (48 RIA edge types) vs.\ $H{=}4$ (12 MHA
...
RIA ($R{=}K{=}4$) \textbf{(ours)} &  \textbf{0.90}     &  \textbf{0.04}          & \textbf{22.5$\times$} \\
\bottomrule
\end{tabular}
\end{table}

The top three RIA edges (Layer~1) each route ~0.90 of the answer's
...
each distractor. RIA's typed structure thus exposes the binding
...
do not factor the endpoints into aspects. RIA's $(r, p, q)$ edge is a
strict superset.
...
composition; Thm.~\ref{thm:rf} shows RIA recovers these as the $R{=}1$
special case.

\paragraph{Part-whole hierarchies.} Capsule networks
\citep{sabour2017dynamic} and GLOM \citep{hinton2021glom} factor the
token into parts; RIA factors the \emph{edge}. The two priors
compose.
...
\paragraph{Frame semantics, description logic, and SRL in neural
networks.} Frame semantics \citep{fillmore1976,baker1998framenet} and
PropBank-style semantic role labelling \citep{palmer2005propbank,
gildea2002automatic} have been the dominant frameworks for typed
relational structure in NLP. They have inspired
representation-learning architectures
\citep{russin2019compositional, webb2020emergent} but, to our
knowledge, the typed-edge factorisation has not previously been
imported into the attention layer of a transformer in the form
proposed here.
...
hyperparameters that work well for it; RIA still strictly dominates.
...
RIA reaches 100.0\% on every seed at $K \in \{2, 4\}$.
...
\textbf{Scope of the claim.}\,Our motivation references frame
semantics, description logic, and semantic role labelling, but
neither the architecture nor the experiments depend on any specific
linguistic theory. Frame semantics provides a clean mapping; the
contribution stands as a neural method on its experimental merits.

\textbf{Scale.}\,Our largest measurement is at 44M parameters on a
Tesla T4 GPU. The ordering of methods is consistent across the six
parameter scales we tested ($\sim$0.6M, 8M, 11M, 14M, 17M, 44M), but
we have not verified that it persists at frontier-scale ($\geq$1B
parameters) or on natural-language pre-training corpora.
...
a single scalar. Importing the role-typing factorisation from
frame-semantic, we defined Role-Indexed Attention and showed it (i)
...
material. The whole RIA layer is five \texttt{einsum} operations plus
...
\section{Weight-copy equivalence of RIA($K{=}1$) and MHA}
\label{app:reduction-mha}

We copy an MHA layer's parameters into an RIA($K{=}1, R{=}H, d_r{=}d_h$)
...
We give a constructive proof. The argument has two parts: (a) RIA at
...
\textbf{RIA realises $f$ exactly at $K{=}2$, $R{=}2$, $d_r{=}d/2$.}
...
\textbf{RIA realises the joint match}, and thus exact retrieval, by
...
output projections $W^O_{r, p}$. With $K{=}2$, $R{=}2$, RIA
...
\section{Top-$k$ role-type gating}
\label{app:topk}
...
attention masses for both RIA (48 edge types) and MHA (12 heads). The
top three RIA edges by retrieval-correct mass all live in Layer~1 and
...
main task. The qualitative pattern --- RIA dominating MHA at
...
Three seeds. The ``MHA wide'' variant matches RIA's parameter
count.}
\label{tab:scaling}
\begin{tabular}{lrcc}
\toprule
...
\hspace{1em} MHA  ($d{=}256$)         & 4,756,992 & 33.9 $\pm$ 1.1 & 33.5 $\pm$ 1.0 \\
\hspace{1em} MHA wide ($d{=}356$)  & 9,178,392 & 33.9 $\pm$ 1.0 & 33.0 $\pm$ 0.4 \\
\hspace{1em} \textbf{RIA} ($K{=}4$)        & 8,295,936 & 88.9 $\pm$ 18.7 & 86.7 $\pm$ 19.3 \\
\midrule
\multicolumn{4}{l}{\textit{Binding (strict-OOD)} ($d{=}256$, $L{=}6$)} \\
\hspace{1em} MHA  ($d{=}256$)         & 4,756,992 & 32.8 $\pm$ 0.9 & 32.9 $\pm$ 1.9 \\
\hspace{1em} MHA wide ($d{=}356$)  & 9,178,392 & 33.8 $\pm$ 1.3 & 33.9 $\pm$ 1.2 \\
\hspace{1em} \textbf{RIA} ($K{=}4$)        & 8,295,936 & 87.3 $\pm$ 23.1 & 85.9 $\pm$ 22.7 \\
\midrule
...
\hspace{1em} \textbf{RIA} ($K{=}4$)        & 8,295,936 & 37.5 $\pm$ 1.8 & 38.5 $\pm$ 1.9 \\
\midrule
...
\hspace{1em} MHA  ($d{=}256$)         & 6,334,464 & 40.0 $\pm$ 0.7 & 39.4 $\pm$ 0.2 \\
\hspace{1em} MHA wide ($d{=}356$)  & 12,226,464 & 41.9 $\pm$ 2.6 & 41.5 $\pm$ 2.0 \\
\hspace{1em} \textbf{RIA} ($K{=}4$)        & 11,053,056 & 48.3 $\pm$ 1.2 & 48.2 $\pm$ 0.9 \\
\midrule
...
\hspace{1em} MHA  ($d{=}256$)         & 7,911,936 & 43.3 $\pm$ 2.0 & 44.2 $\pm$ 2.8 \\
\hspace{1em} MHA wide ($d{=}356$)  & 15,274,536 & 47.5 $\pm$ 1.3 & 47.0 $\pm$ 0.7 \\
\hspace{1em} \textbf{RIA} ($K{=}4$)        & 13,810,176 & 65.0 $\pm$ 7.7 & 66.1 $\pm$ 10.1 \\
\midrule
...
\hspace{1em} \textbf{RIA} ($K{=}4$)        & 16,567,296 & 70.6 $\pm$ 8.4 & 69.6 $\pm$ 10.4 \\
\midrule
...
\hspace{1em} \textbf{RIA} ($K{=}4$)        & 44,126,208 & 34.2 $\pm$ 0.4 & 33.9 $\pm$ 1.0 \\
\midrule
...
\hspace{1em} \textbf{RIA} ($K{=}4$)        & 44,126,208 & 37.2 $\pm$ 2.2 & 38.4 $\pm$ 0.9 \\
\bottomrule
\end{tabular}
\end{table}
